...
@Component({
  ...
  styles: [`
    ...
    .gl-hidden { opacity: 0; visibility: hidden; pointer-events: none; }
    .gl-card { display: flex; flex-direction: column; align-items: center; gap: 1.5rem; }
    .gl-logo {
      font-family: 'Syne', sans-serif; font-weight: 800; font-size: 2rem;
      color: #f0ede8; letter-spacing: -.03em;
    }
    .gl-dot { color: #f5a623; }
    .gl-ring-wrap { position: relative; width: 52px; height: 52px; }
    .gl-ring {
      position: absolute; inset: 0; border-radius: 50%;
      border: 2.5px solid transparent; border-top-color: #f5a623;
      animation: gl-spin .9s linear infinite;
    }
    .gl-ring-2 {
      inset: 8px; border-top-color: rgba(245,166,35,.35);
      animation-duration: 1.4s; animation-direction: reverse;
    }
    .gl-label {
      font-family: 'Space Mono', monospace; font-size: .65rem;
      letter-spacing: .2em; text-transform: uppercase; color: #7a7570;
    }
    .gl-bar-track { width: 120px; height: 2px; background: #252525; overflow: hidden; }
    .gl-bar { height: 100%; background: #f5a623; animation: gl-pulse 1.6s ease-in-out infinite; }
